...
class DatabaseHandler (ctx: Context): SQLiteOpenHelper(ctx,DB_NAME,null,DB_VERSION) {
    ...
    fun addPessoa(pessoa: Detail): Boolean{
    ...
    fun pessoas(): ArrayList<Detail> {
        val pessoaList = ArrayList<Detail>()
        val db = writableDatabase
    ...
    fun updatePessoa(pessoa: Detail): Boolean{
        val db = this.writableDatabase
    ...
    companion object {
        ...
        private val DB_NAME = "Cloude"
        private val TABLE_NAME = "detail"
        private val ID = "Id"
